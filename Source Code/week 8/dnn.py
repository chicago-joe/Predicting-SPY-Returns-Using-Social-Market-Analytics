...
for layer1 in range(10,100,10):
    for layer2 in range(10,100,10):
        for layer3 in range(10,100,10):
            model = Sequential()
            model.add(Dense(layer1, init='uniform', input_dim=27))
...
#model.compile(loss='mean_squared_error', optimizer="adam", metrics=["accuracy",Accuracy_approx])
            ...
            table["layer1:%i"%layer1+" layer2:%i"%layer2+" layer3:%i"%layer3]=[up_dir,down_dir,up_dir_y,down_dir_y,up_dir_pred,down_dir_pred]


model = Sequential()
model.add(Dense(20, init='uniform', input_dim=27))
model.add(Activation('linear'))
...
epsilon=0.0000001
for i in table.keys():
    precision.append(table[i][0]/(table[i][4]+epsilon))
    recall.append(table[i][0]/table[i][2])
    key.append(i)
